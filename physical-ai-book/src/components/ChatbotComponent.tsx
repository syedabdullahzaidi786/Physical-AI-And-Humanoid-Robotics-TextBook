...
export default function ChatbotComponent({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      text: "Hello! I am your Physical AI And Humanoid Robotics Assistant. Ask me anything about the course!",
      sender: "bot",
    },
  ]);
      ...
      if (selection && selection.toString().trim().length > 0) {
        ...
        try {
          const response = await fetch(
            "https://syedabdullahzaidi786-rag-chatbot-backend.hf.space/api/ask",
            {
              method: "POST",
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await fetch(
        "https://sobansaud028382-huggingnewback.hf.space/api/ask",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
